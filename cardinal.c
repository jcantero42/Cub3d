...
void	ea_pixel_put(t_game *g, int i, int x)
{
	...
	pixel_put(g, x, i, pixel_get(g->ea_img, round(g->ea_img->width * x_ratio), round(g->ea_img->height * y_ratio)));
...
void	we_pixel_put(t_game *g, int i, int x)
{
	...
	y_ratio = (i - (WIN_HEIGHT / 2.0 - g->h / 2.0)) / (double)g->h;
	x_ratio = 0.5 - (round(g->vy) - g->vy);
	pixel_put(g, x, i, pixel_get(g->we_img, round(g->we_img->width * x_ratio), round(g->we_img->height * y_ratio)));
